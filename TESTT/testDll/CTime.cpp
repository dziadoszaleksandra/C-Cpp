...
int Time::setMinute( int nMinute ) {
    if( !(nMinute >= 0 && nMinute < 60) ) return 1;
    this->m_nMinute = nMinute;
    return 0;
}

int Time::setSecond( int nSecond ) {
    if( !(nSecond >= 0 && nSecond < 60) ) return 1;
...
std::ostream& operator<<( std::ostream& out, const Time& t ) {
    int n;
    if( (n = t.getHour()) < 10 ) out << 0;
    out << n << ':';
    if( (n = t.getMinute()) < 10 ) out << 0;
    out << n << ':';
    if( (n = t.getSecond()) < 10 ) out << 0;
    out << n;
    return out;
}